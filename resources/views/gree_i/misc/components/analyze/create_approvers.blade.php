...
<div class="modal fade" id="modal_office_approv" tabindex="-1" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title white">Cargo aprovador</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="bx bx-x"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-12 col-md-12 col-sm-12">        
                        <input type="hidden" id="user_index_office">
                        <div class="form-group">
                            <label>Selecione o cargo do aprovador</label>
                            <select class="form-control" style="width: 100%;" id="select_office_mark"></select>
                        </div>
                    </div>    
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary ml-1" id="btn_add_office">
                    <i class="bx bx-check d-block d-sm-none"></i>
                    <span class="d-none d-sm-block">Adicionar</span>
                </button>
            </div>
...
<form class="form" action="/misc/components/analyze/create/approvers_do" method="post" id="form_visitor">
...
<script>
    ...
    var arr_mark = {!! json_encode($arr_mark) !!};
    var activemenu = {!! json_encode($activemenu) !!};
    var arr_approv_delete = [];
    var arr_observers_delete = [];

    $(document).ready(function (state) {
        
        if(Object.keys(arr_mark).length > 0) {
            $("#select_office_mark").html(reloadOfficeMark(arr_mark));
        }
        ...
        $(document).on('click', '.add-office-mark', function (e) {
            var index = parseInt($(this).attr("data-index"));
            $("#user_index_office").val(index);
            $("#modal_office_approv").modal('show');
        });

        $("#btn_add_office").click(function() {

            var index = $("#user_index_office").val();
            var value = $("#select_office_mark").val();

            if(arr_approv[index] != undefined) {
                arr_approv[index].mark = parseInt(value);
                if(arr_approv[index].arr_approvers) {
                    arr_approv[index].arr_approvers.forEach(function(item, index) {
                        item.mark = parseInt(value);
                    });
                }
                $('#list_approv').html(reloadApprov(arr_approv));
                $("#select_office_mark").val("");
                $("#modal_office_approv").modal('hide');

            } else {
                $error('Erro ao adicionar cargo no aprovador!');
            }
        });

        function reloadOfficeMark(arr_mark) {
            var html = '<option value="" selected disabled>Selecione o cargo</option>';
            Object.keys(arr_mark).forEach(function(key){
                html += '<option value="'+key+'">'+arr_mark[key]+'</option>';
            });
            return html;
        }
        ...
        function reloadApprov(object) {
            ...
            if(object.length > 0) {
                ...
                for (var i = 0; i < object.length; i++) {
                    ...
                    var mark = row.mark != 1 ? '- ' + arr_mark[row.mark] : '';
                    var position = i + 1;
                    
                    html += `<li class="list-group-item list-group-item-action handle" data-index="`+i+`">
                                <div class="list-left d-flex">
                                    <i class="bx bx-grid-vertical cursor-move handle " style="margin-top: 10px; margin-right: 10px; margin-left: -10px;"></i>
                                    <div class="list-icon mr-1">
                                        <div class="avatar bg-rgba-primary m-0">
                                            <img class="" src="`+ picture +`" alt="img placeholder" height="38" width="38">
                                        </div>
                                    </div>
                                    <div class="list-content">
                                        <span class="list-title text-bold-500">`+ row.name +`</span>
                                        <small class="text-muted d-block">`+ position +`° Aprovador `+ mark +`</small>
                                    </div>
                                    <div style="right: 70px; position: absolute; top: 23px;">
                                        <i class="bx bx-bookmark-plus font-medium-1 add-office-mark" style="font-size: 1.40rem !important; cursor:pointer;" data-index="`+i+`"></i>
                                    </div>
                                    <div style="right: 35px; position: absolute; top: 23px;">
                                        <i class="bx bx-user-plus font-medium-1 add-approv-position" style="font-size: 1.40rem !important; cursor:pointer;" data-index="`+i+`"></i>
                                    </div>
                                    <div style="right: 10px; position: absolute; top: 25px;">
                                        <i class="bx bx-trash delete-approv font-medium-1" style="color:#ff6060; cursor:pointer;" data-index="`+i+`"></i>
                                    </div>
                                </div>`;
                            ...
                            html += `<div class="list-left d-flex mt-1 ml-2">
                                        <div class="list-icon mr-1">
                                            <div class="avatar bg-rgba-info m-0">
                                                <img class="" src="`+ picture2 +`" alt="img placeholder" height="38" width="38">
                                            </div>
                                        </div>
                                        <div class="list-content">
                                            <span class="list-title text-bold-500">`+ row_sub.name +`</span>
                                            <small class="text-muted d-block">`+ position +`° aprovador `+ mark +`</small>
                                        </div>
                                        <div style="position: absolute; right: 10px; margin-top: 5px;">
                                            <i class="bx bx-trash delete-sub-approv font-medium-1" style="color:#ff6060; cursor:pointer;" data-index="`+ i +`" data-sub-index="`+ j +`"></i>
                                        </div>
                                    </div>`;
                        }
                    }
        ...
        setTimeout(() => {
            var arr_active = activemenu.split(',');
            if(arr_active.length > 1) {
                $('#'+arr_active[0]+'').addClass('sidebar-group-active active');                
                $('#'+arr_active[1]+'').addClass('sidebar-group-active active');
                if(arr_active.length > 2) {
                    $('#'+arr_active[2]+'').addClass('active');
                }
            }
        }, 100);
    });
