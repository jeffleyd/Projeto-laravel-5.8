                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table" id="table_cost">
                                    ...
                                    <tbody>
                                        @if ($process_cost->count() > 0)
                                            @foreach ($process_cost as $cost)
                                                <tr role="row">
                                                    ...
                                                    <td><a data-toggle="tooltip" data-placement="left" data-original-title="<?= $cost->juridical_process->process_number ?>" href="/juridical/process/info/<?= $cost->juridical_process_id ?>"><?= stringCut($cost->juridical_process->process_number, 15) ?></a></td>
                                                    <td><?= date('d/m/Y', strtotime($cost->date_release)) ?></td>
                                                    <td><span data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= stringCut($cost->juridical_type_cost->description, 200) ?>" style="cursor: pointer;"><?= stringCut($cost->juridical_type_cost->description, 15) ?></span></td>
                                                    <td><span data-toggle="tooltip" data-placement="left" title="" data-original-title="<?= stringCut($cost->description, 200) ?>" style="cursor: pointer;"><?= stringCut($cost->description, 16) ?></span></td>
                                                    <td>
                                                        @if($cost->total != 0)
                                                            R$ <?= number_format($cost->total, 2,",",".") ?>
                                                        @else
                                                            <span class="badge badge-warning badge-pill" style="padding: 0.5rem; font-size: 0.6rem; line-height: 0.6;">Sem comprovante</span>
                                                        @endif
                                                    </td>
                                                    <td><?= date('d/m/Y', strtotime($cost->date_expiration)) ?></td>
                                                    <td>
                                                        @if($cost->is_paid == 0)
                                                            <span class="badge badge-light-danger badge-pill span-status-paid" data-payment-cost-id="<?= $cost->id ?>">Não pago</span>
                                                        @elseif($cost->is_paid == 1)
                                                            <span class="badge badge-light-success badge-pill">Pago</span>
                                                        @endif    
                                                    </td>
                                                    <td>
                                                        <div class="dropleft">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                @if($cost->is_paid == 0)
                                                                    <a class="dropdown-item edit-cost" href="/juridical/process/info/<?= $cost->juridical_process_id ?>?tabName=cost" data-cost-id="<?= $cost->id ?>"><i class="bx bx-edit-alt mr-1"></i> Adicionar comprovante</a>
                                                                @endif
                                                                @if($cost->total != 0)
                                        ...
                                        @else    
                                        <tr role="row">
                                            <td colspan="9">Não custos de processos cadastrados!</td>
                                        </tr>    
                                        @endif 
                                    </tbody>
                                </table>
                                <nav>
                                    <ul class="pagination justify-content-end">
                                        <?= $process_cost->appends([
                                            'code_cost' => Session::get('cost_code_cost'),
                                            'cost_date_release_submit' => Session::get('cost_cost_date_release_submit'),
                                            'type_cost' => Session::get('cost_type_cost'),
                                            'cost_date_expiration_submit' => Session::get('cost_cost_date_expiration_submit'),
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-primary white">
                <span class="modal-title">Filtrar Custos</span>
            </div>
            <div class="modal-body">
                <form action="{{Request::url()}}" id="form_modal_filter">
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label for="first-name-vertical">Número processo</label>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title white" id="myModalLabel160">Notas fiscais</h5>
...
<script>
    $(document).ready(function () {
        ...
        setInterval(() => {
            ...
            $("#mJuridicalProcessCost").addClass('active');
        }, 100);
    });
