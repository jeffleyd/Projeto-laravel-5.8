            ...
            <div class="users-list-table">
                <div class="card">
                    <div class="card-content">
                        <form action="/juridical/type/action/list" method="GET">
                            <div class="card-body">
                                <div class="top d-flex flex-wrap">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control"  name="description" id="filter_description" placeholder="Filtrar Ação">
                                                    </div>
                                                </div>    
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                    ...
                                    <div class="actions action-btns d-flex align-items-center">
                                        ...
                                        <div class="dropdown invoice-options">
                                            <button type="button" class="btn btn-success shadow mr-1" onclick="typeEdit(0, 1, '', '')"><i class="bx bx-plus-circle"></i> Nova Ação</button>
                                        </div>
                                    </div>
                                </div>
                            </div> 
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="list-datatable" class="table">
                                    ...
                                    <tbody>
                                        <?php foreach ($type_action as $key) { ?>
                                        <tr>
                                            <td><?= $key->id ?></td>
                                ...
                                <nav aria-label="Page navigation">
                                    <ul class="pagination justify-content-end">
                                        <?= $type_action->appends([
                                                'description' => Session::get('type_action_description'),
                                                'status' => Session::get('type_action_status'),
                                            ])->links(); 
                                        ?>
                                    </ul>
                                </nav>
...
<div class="modal fade text-left" id="modal-update" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <form id="modal_form_type" method="POST" action="/juridical/type/action/edit_do">
...
<script>
    ...
    $(document).ready(function () {
        ...
        setInterval(() => {
            ...
            $("#mJuridicalTypeAction").addClass('active');
        }, 100);
    });
    ...
    function typeEdit(id, is_new, description, status) {
        ...
        if (is_new == 1) {
            $(".modal-title").html("Nova ação");
            $("#description").val('');

            $('#status option[value="1"]').attr('selected','selected');
            $(".div-modal-status").hide();
            //$(".div-modal-status").prop('disabled', 'disabled');

        } else {
            $(".modal-title").html("Editando ação");
