...
<div class="content-wrapper">
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2 mt-1">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h5 class="content-header-title float-left pr-1 mb-0">{{ __('trip_i.tn_trip_edit_plan') }} #<?= $id ?></h5>
                    <div class="breadcrumb-wrapper col-12">
                        {{ __('trip_i.tn_trip_edit_plan_sub') }}
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <ul class="nav nav-tabs nav-justified mb-3" id="myTab2" role="tablist">
                                  ...
                                  <div class="tab-content pt-1">
                                    <div class="tab-pane active" id="home-just" role="tabpanel" aria-labelledby="home-tab-justified">
                                        <form action="/trip/update/<?= $id ?>" method="POST" class="wizard-horizontal" id="PlanTrip">
  ...
  <button id="FinishPlan" type="submit" class="btn btn-success trip-btn-action glow px-1"><?= __('trip_i.tn_finish') ?></button>
...
<script>
    ...
    function ReloadTablePlan() {
      ...
      for(var i = 0; i < ArrayPlan.length; i++) {
          ...
          if (arrayObj.has_disable == 1) {
            plan += '<tr class="table-primary">';
          } else if (arrayObj.is_approv == 1) {
            plan += '<tr class="table-success">';
          } else if (arrayObj.has_analyze == 1) {
            plan += '<tr class="table-warning">';
          } else if (arrayObj.is_reprov == 1) {
            plan += '<tr class="table-danger">';
          } else {
            plan += '<tr>';
          }
          ...
          plan += '<td class="text-center">';
        if (arrayObj.has_analyze == 0) {
            if (arrayObj.can_edit == 1 || arrayObj.has_disable == 0) {
    ...
    $(document).ready(function () {
        ...
        <?php foreach ($trips as $key) { ?>
            <?php $peoples = App\Model\TripPeoples::where('trip_plan_id', $key->id)->get(); ?>
            <?php if (count($peoples) > 0) {  ?>
                <?php foreach ($peoples as $people) { ?>
                    ArrayPeoples.push({
                        "r_code": <?= $people->identity ?>,
                        "name": "<?= $people->name ?>",
                    });
            ...
            ArrayPlan.push({
                        "id" : <?= $key->id ?>,
                        "finality" : <?= $key->finality ?>,
                        "other" : "<?= $key->other ?>",
                        "goal" : "<?= preg_replace( "/\r|\n/", " ", $key->goal); ?>",
                        "is_approv" : <?= $key->is_approv ?>,
                        "is_reprov" : <?= $key->is_reprov ?>,
                        "has_analyze" : <?= $key->has_analyze ?>,
                        "has_disable" : <?php $has_budget = App\Model\TripAgencyBudget::where('trip_plan_id', $key->id)->first(); if ($has_budget) { echo 1; } else { echo 0; } ?>,
                        "can_edit" : <?php if (App\Model\UserOnPermissions::where('user_r_code', Session::get('r_code'))->where('perm_id', 1)->where('grade', 99)->count() > 0) { echo 1; } else { echo 0; } ?>,
                        "flight" : {
                            "origin_date": "<?= date('Y-m-d', strtotime($key->origin_date)) ?>",
                            "origin_period": <?= $key->origin_period ?>,
                            "origin_country": <?= $key->origin_country ?>,
                            "origin_country_name": "<?= GetCountryName($key->origin_country) ?>",
                            "origin_state": <?= $key->origin_state ?>,
                            "origin_state_name": "<?= GetStateName($key->origin_country, $key->origin_state) ?>",
                            "origin_city": "<?= $key->origin_city ?>",
                            "destiny_date": "<?= date('Y-m-d', strtotime($key->destiny_date)) ?>",
                            "destiny_period": <?= $key->destiny_period ?>,
                            "destiny_country": <?= $key->destiny_country ?>,
                            "destiny_country_name": "<?= GetCountryName($key->destiny_country) ?>",
                            "destiny_state": <?= $key->destiny_state ?>,
                            "destiny_state_name": "<?= GetStateName($key->destiny_country, $key->destiny_state) ?>",
                            "destiny_city": "<?= $key->destiny_city ?>",
                            "dispatch": <?= $key->dispatch ?>,
                            "dispatch_info": "<?= $key->dispatch_reason ?>",
                        },
                        "has_hotel" : <?= $key->has_hotel ?>,
                        "hotel" : {
                            "enter_date": "<?= date('Y-m-d', strtotime($key->hotel_date)) ?>",
                            "checkout": <?= $key->hotel_checkout ?>,
                            "enter_country": <?= $key->hotel_country ?>,
                            "enter_country_name": "<?= GetCountryName($key->hotel_country) ?>",
                            "enter_state": <?= $key->hotel_state ?>,
                            "enter_state_name": "<?= GetStateName($key->hotel_country, $key->hotel_state) ?>",
                            "enter_city": "<?= $key->hotel_city ?>",
                            "exit_date": "<?= date('Y-m-d', strtotime($key->hotel_exit)) ?>",
                            "address": "<?= $key->hotel_address ?>",
                        },
                        <?php if (count($peoples) > 0) {  ?>
