            ...
            <form action="{{Request::url()}}" id="searchTrip" method="GET">
                <div class="row border rounded py-2 mb-2">
					...
					<div class="col-12 col-sm-12 col-lg-3">
                        <label for="code">Pesquisar na descrição</label>
                        <fieldset class="form-group">
                            <input type="text" class="form-control" id="description" value="{{Session::get('filter_description')}}" name="description">
                        </fieldset>
                    </div>
                                ...
                                <table id="list-datatable" class="table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><span data-toggle="popover" data-content="<?= $key->description ?>"><?= \Str::limit($key->description, 35); ?></span></td>
                                            <td>
                                                <div class="dropleft">
                                                    <span class="bx bx-dots-horizontal-rounded font-medium-3 nav-hide-arrow cursor-pointer" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" role="menu"></span>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a class="dropdown-item" href="/administration/generic/request/view?s={{$key->hash_code}}"><i class="bx bx-show mr-1"></i> Visualizar</a>
                                                        <a class="dropdown-item" target="_blank" href="{{$key->AdmRequestFiles->url}}"><i class="bx bx-printer mr-1"></i> Imprim. Solic.</a>
                                                        @if ($key->is_approv == 0 and $key->is_reprov == 0 and $key->is_cancelled == 0)
                                                        <a class="dropdown-item" onclick="cancel('{{$key->hash_code}}')" id="cancel" href="javascript:void(0)"><i class="bx bx-x mr-1"></i> Cancelar</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
...
<script>
    function cancel($hash) {
        var r = confirm("Você realmente deseja cancelar a solicitação?");
        if (r == true) {
            block();
            window.location.href = '/administration/generic/request/cancelled?s='+$hash;
        }
    }
