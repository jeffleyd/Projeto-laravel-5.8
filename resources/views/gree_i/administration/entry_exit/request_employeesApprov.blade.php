                            ...
                            <div class="top d-flex flex-wrap">
                                <div class="action-filters flex-grow-1">
                                    <div class="dataTables_filter mt-1">
                                        <h5>Lista de solicitações para aprovação</h5>
                                    </div>
                                </div>
                                            ...
                                            <td id="action">
                                                <div class="dropleft">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        ...
                                                        <a onclick="analyze(1, {{$key->id}})" href="javascript:void(0)" class="dropdown-item"><i class="bx bx-check-circle mr-1"></i> Aprovar</a>
                                                        <a onclick="analyze(2, {{$key->id}})" href="javascript:void(0)" class="dropdown-item"><i class="bx bx-x-circle mr-1"></i> Reprovar</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
...
<div class="mb-2" style="position: fixed;left: 0;right: 0;bottom: 0;margin: 0 auto;z-index: 99;text-align: center;">
  <button type="button" onclick="analyze(1, 0)" class="btn btn-secondary">Aprovar vários</button>
  <button type="button" onclick="analyze(2, 0)" class="btn btn-danger">Reprovar vários</button>
</div>
<script>
    function analyze(type, id) {
			var selIDs = [];
			document.getElementsByName('check[]').forEach(function($item){
				if ($item.checked)
					selIDs.push($item.value);
			});
			if (id == 0 && selIDs.length == 0) {
				$error('Você precisa selecionar ao menos 1 para fazer essa operação.');
				return;
			}
            Swal.fire({
                title: type === 1 ? 'Aprovar solicitação' : 'Reprovar solicitação',
                text: "Para dar andamento, informe sua senha e confirme.",
                type: 'warning',
                input: 'password',
                ...
                if (result.value) {
                    ...
                    ajaxSend(
                        '/adm/entry-exit/approv/employees_do',
                        {
                            'id': selIDs.length > 0 ? selIDs.join() : id,
                            'type': type,
        ...
        html += `<table class="table table-bordered table-view">
                        ...
                        <tr>
                            ...
                            <td class="td-font-14" colspan="9">
                                ${data.reason === 1 ? 'Serviço' : 'Particular'}
                            </td>
                        </tr>
                        <tr>
                            <td class="td-title td-font-14 td-bold" colspan="2">
    ...
    $(document).ready(function () {
        ...
        setInterval(() => {
                    ...
                    $("#mEntryExitApprov").addClass('active');
                }, 100);
