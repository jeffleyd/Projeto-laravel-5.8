                            ...
                            <div class="top d-flex flex-wrap">
                                <div class="action-filters flex-grow-1">
                                    <div class="dataTables_filter mt-1">
                                        <h5 >Custos Remessas de peças</h5>
                                    </div>
                                </div>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Total<i class="bx bx-info-circle cursor-pointer" 
                                                ...
                                                data-toggle="tooltip" data-placement="bottom" data-original-title="TOTAL = VALOR PEÇAS + VALOR FRETE + VALOR DO SERVIÇO"></i>
											</th>
											<th>Pagam. Serviço<i class="bx bx-info-circle cursor-pointer" 
                                                style="color: #3568df; position: relative; bottom: 5px; font-size: 0.9rem;"
                                                data-toggle="tooltip" data-placement="bottom" data-original-title="VALOR DA SOLICITAÇÃO DE PAGAMENTO DO SERVIÇO"></i>
                                            </th>
                                            <th>Atualizado</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($costs as $i => $key)
                                            ...
                                            <tr class="cursor-pointer showDetails">
                                                ...
                                                <td>R$ {{ number_format($key->value_service_total, 2, ',', '.') }}</td>
                                                <td style="color: #4e4141;">R$ {{ number_format($key->cost_total, 2, ',', '.') }}</td>
												<td>R$ {{ number_format($key->total_payment, 2, ',', '.') }}</td>
                                                                            ...
                                                                            <tbody>
                                                                                ...
                                                                                <tr>
                                                                                    <td style="padding:0px;">
                                                                                        @if($part->remittance_cost_field()->observation != "")
                                                                                            <i class="bx bx-info-circle modal-details" data-observation="{{ $part->remittance_cost_field()->observation }}" style="cursor: pointer; color:#3568df;" data-toggle="tooltip" data-placement="left" data-original-title="Clique para visualizar a observação"></i>
                                                                                        @else 
                                                                                    ...
                                                                                    <td>
                                                                                        @if(isset($part->parts['description'])) {{ $part->parts['description'] }} <b>({{$part->parts['code']}})</b> @else {{ $part->part }} <br><b style="color:#d24e4e;">(Solic. Manual)</b> @endif
                                                                                    ...
                                                                                    <td style="font-size: 14px;">
																						{{ $part->remittance_cost_field()->date_billing ? date('d/m/Y', strtotime($part->remittance_cost_field()->date_billing)) : '00/00/0000' }}
																						({{ $part->remittance_cost_field()->hour_billing ? $part->remittance_cost_field()->hour_billing : '00:00'}})
																					</td>
                                                                                    <td>{{ $part->remittance_cost_field()->number_order }}</td>
                                                                                    <td>{{ $part->remittance_cost_field()->number_di }}</td>
                                                                                    <td>{{ $part->remittance_cost_field()->number_nf }}</td>
                                                                                    <td style="font-size: 14px;">
																						{{ $part->remittance_cost_field()->date_emission_nf ? date('d/m/Y', strtotime($part->remittance_cost_field()->date_emission_nf)) : '00/00/0000' }}
																						({{ $part->remittance_cost_field()->hour_emission_nf ? $part->remittance_cost_field()->hour_emission_nf : '00:00' }})
																					</td>
                                                                                    <td>R$ {{ number_format($part->remittance_cost_field()->value_part, 2, ',', '.') }}</td>
                                                                                    <td>R$ {{ number_format($part->remittance_cost_field()->value_shipping, 2, ',', '.') }}</td>
                                                                                    <td>R$ {{ number_format($part->service_value, 2, ',', '.') }}</td>
                                                                ...
                                                                <div class="tab-pane" id="tab-model-{{$i}}" role="tabpanel" aria-labelledby="profile-tab-fill">
                                                                    <div class="table-responsive">
                                                                        <table class="table">
                                                                            ...
                                                                            <tbody>
                                                                                @foreach ($key->sac_remittance_parts->unique('model') as $model)
                                                                                    <tr>
                                                                                        <td>@if(isset($model->product_air['model'])) {{ $model->product_air['model'] }} @else {{ $model->model }} @endif</td>
                                                                                        <td>@if(isset($model->product_air['serial_number'])) {{ $model->product_air['serial_number'] }} @else {{ $model->serial_number }} @endif</td>
                                                                                    </tr>    
                                                                                @endforeach
                                                                            </tbody>    
                                                                        </table>
                                ...
                                <nav>
                                    <ul class="pagination justify-content-end">
                                        <?= $costs->appends([
                                            ...
                                            'begin_date_remit_submit' => Session::get('cost_begin_date_remit_submit'),
                                            'end_date_remit_submit' => Session::get('cost_end_date_remit_submit')
                                        ])->links(); ?>   
                                    </ul>
                                </nav>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="/sac/assistance/warranty/remittance/costs/edit_do" id="register_cost">
                    <input type="hidden" name="cost_id" id="cost_id" value="0">
                    <input type="hidden" name="remittance_id" id="remittance_id">
                    <input type="hidden" name="part_remittance_id" id="part_remittance_id">
                    <div class="row">
                        <div class="col-12">
            ...
            <div class="modal-body">
                <form action="{{Request::url()}}" id="form_modal_filter">   
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                <label for="first-name-vertical">Data Inicial Emissão Remessa</label>
                                <input type="text" class="form-control date-format" name="begin_date_remit" placeholder="00/00/0000">
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                <label for="first-name-vertical">Data Final Emissão Remessa</label>
                                <input type="text" class="form-control date-format" name="end_date_remit" placeholder="00/00/0000">
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="code_remittance" placeholder="W00000">
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
...
<script>
	...
	function confirmInvoice($this) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                block();
                window.location.href = '/sac/assistance/warranty/remittance/part/invoice/' + id;
            }
        });
    }
    ...
    function editCost($this) {
        ...
        $('#remittance_id').val(obj.sac_remittance_part_id);
        $("#part_remittance_id").val(obj.id);
        $(".input-cost").val('');

        if(obj.sac_remittance_part_costs) {
            $("#number_order").val(obj.sac_remittance_part_costs.number_order);
            $("#number_di").val(obj.sac_remittance_part_costs.number_di);
            $("#number_nf").val(obj.sac_remittance_part_costs.number_nf);
            $("#date_emission_nf").val(obj.sac_remittance_part_costs.date_emission_nf);
			$("#hour_emission_nf").val(obj.sac_remittance_part_costs.hour_emission_nf);
            $("#date_billing").val(obj.sac_remittance_part_costs.date_billing);
			$("#hour_billing").val(obj.sac_remittance_part_costs.hour_billing);
			$("#value_part").val(numberToReal(obj.sac_remittance_part_costs.value_part));
            $("#value_shipping").val(numberToReal(obj.sac_remittance_part_costs.value_shipping));
            $("#cost_id").val(obj.sac_remittance_part_costs.id);
            $("#update_reason").val(obj.sac_remittance_part_costs.update_reason);
            $("#observation").val(obj.sac_remittance_part_costs.observation);
			$("#value_shipping_return").val(numberToReal(obj.sac_remittance_part_costs.value_shipping_return));
			
			if(obj.sac_remittance_part_costs.update_reason == 2) {
                $(".div-shipping-return").show();
            } else {
                $(".div-shipping-return").hide();
                $("#value_shipping_return").val('');
    ...
    function historicCost($this) {
        ...
        if(obj.sac_remittance_part_costs) {
            ...
            object = obj.sac_remittance_part_costs.sac_remittance_part_costs_historic
