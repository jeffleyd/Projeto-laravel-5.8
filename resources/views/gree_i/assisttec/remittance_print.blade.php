    ...
    <div id="defesa">
        ...
        <div id="conteudo">
            ...
            <div id="topo_os" style="float:left;border-bottom-width:1px;border-bottom-color:#000;border-bottom-style:solid;">
                ...
                <div id="dados_doc">
                    <h2><span>N°</span> <strong><?= $remittance->code ?></strong></h2>
                    <p><span>Cód. do Credenciado:</span> <strong><?= $authorized->code ?></strong></p>
                    <p class="menor"><span>Data de Emissão:</span> <strong><?= date('d/m/Y', strtotime($remittance->created_at)) ?></strong></p>
                </div>
            ...
            <div id="bloco">
                ...
                @if (count($remitance_parts) > 0)              
                    <div class="limpar"></div>
                    <div class=" fl-left" style=" width: 13%; text-align: center; background-color: #ccc; padding: 10px">Modelo</div>
                    <div class=" fl-left" style=" width: 20%; text-align: center; background-color: #ccc;  padding: 10px">Código peça</div>
                    <div class=" fl-left" style=" width: 30%; text-align: center; background-color: #ccc;  padding: 10px">Nome peça</div>
                    <div class=" fl-left" style=" width: 30%; text-align: center; background-color: #ccc;  padding: 10px">Motivo Solicitação</div>
                    <div class=" fl-left" style=" width: 7%; text-align: center; background-color: #ccc;  padding: 10px">Quant.</div>
                    
                    <div class="limpar"></div>

                    @foreach ($remitance_parts as $key)
                        <div class=" fl-left" style=" width: 13%; text-align: center; padding: 10px">
                            @if(isset($key->product_air['model'])) {{ $key->product_air['model'] }} @else {{ $key->model }} @endif
                        </div>
                        <div class=" fl-left" style=" width: 20%; text-align: center; padding: 10px">
                            @if(isset($key->parts['code'])) {{ $key->parts['code'] }} @else Manual @endif
                        </div>
                        <div class=" fl-left" style=" width: 30%; text-align: center; padding: 10px">
                            @if(isset($key->parts['description'])) {{ $key->parts['description'] }} @else {{ $key->part }} @endif
                        </div>
                        <div class=" fl-left" style=" width: 30%; text-align: center; padding: 10px"><?= $key->description_order_part ?></div>
                        <div class=" fl-left" style=" width: 7%; text-align: center; padding: 10px"><?= $key->quantity ?></div>
            ...
            <div id="bloco">      
                ...
                <div class="limpar"></div>
                <p style="text-align: center">Não há informações</p>
            </div>
			
			

            <div id="bloco" class="noborder">
                <p>Confirmo a realização do pedido de solicitação de remessa de peça(s).<br />&nbsp;</p>
			...
			<div id="bloco">
                ...
                <div class="limpar"></div>
                <p>{{$remittance->payment_info}}</p>
                <br>
                <span class="box box-larga">
                </span>
                <span class="box box-larga no-margim" style="float: right;text-align: right;">
                    <p class="cinquenta"><span>Custo da mão de obra:</span> <strong>R$ {{number_format($remitance_parts->sum('service_value'), 2, ',', '.')}}</strong></p>
                    <p class="cinquenta"><span>Total do pagamento:</span> <strong>R$ {{number_format($remitance_parts->sum('service_value'), 2, ',', '.')}}</strong></p>
                </span>
                <div class="limpar"></div>
            </div>
