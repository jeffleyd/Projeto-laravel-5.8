    ...
    <div class="col-sm-12">
        ...
        <table class="table table-bordered">
            <tbody>
                ...
                @php
                    $categories = collect(json_decode($order->programationMonth->programation->json_categories_products));
                    $index = 0;
                    ...
                    <tr>
                        @foreach($arr_column as $idx => $column)
                            @if (isset($column[$index]))
                                ...
                                $qtd = 0;
                                $is_price_custom = 0;
								$is_qtd_diff_programation = 0;
								$is_prod_diff_programation = 0;
                                $price_unit = 0.00;
                                $totalg = 0.00;
                                $descoint = 0;
                                if ($item)
                                ...
                                @if ($orderProductf)
                                    @php
										...
										$is_qtd_diff_programation = $orderProductf->is_qtd_diff_programation;
										$is_prod_diff_programation = $orderProductf->is_prod_diff_programation;
                                ...
                                @endif
                                @if ($item)
                                    <td class="td-font-14 td-text-center @if($is_prod_diff_programation == 1) border-alert @endif">
                                    ...
                                    <td class="td-font-14 td-text-center @if($is_qtd_diff_programation == 1) border-alert @endif">
                                        {{$qtd}}
                                    </td>
                ...
                <tr>
                    ...
                    <td class="td-font-14 td-title" colspan="3">
                        @if ($order->orderFinancyAnalyze)
                            DATA: <span style="margin-left: 50px;">{{date('d/m/Y', strtotime($order->orderFinancyAnalyze->created_at))}}</span>
                        @else
                            <span style="float: left;position: absolute;">DATA: </span> <span style="display: flex;justify-content: center;">_____/_____/_____</span>
                        @endif
                    </td>
