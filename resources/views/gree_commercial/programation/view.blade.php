                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        <td><b>Tipo do cliente:</b> <span id="type_client">{{$client->type_client_name}}</span></td>
                                    </tr>
                                    <tr>
                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach ($arr_row as $index => $val)
                                    ...
                                    @if ($val['version'] == $version->version)
                                        <?php $html .= '<td style="cursor: pointer;background-color: #fb9678!important;" onclick="viewVersion('.$val['version'].')" class="text-center bg-primary"><a style="color: white !important" href="javascript:void(0)">Ver: 00'. $val['version'].' <span style="font-size: 10px">'.$val['created'].'</span></a><br><span class="tagst" style="color:white"> '. $status_row .' </span></td>'; ?>
                                    @else
                                        <?php $html .= '<td style="cursor: pointer" onclick="viewVersion('.$val['version'].')" class="text-center"><a href="javascript:void(0)">Ver: 00'. $val['version'].' <span style="font-size: 10px">'.$val['created'].'</span></a><br><span class="tagst" style="color:black"> '. $status_row .' </span></td>'; ?>
                                    @endif
                                    <?php $trow++; ?>
                                    @if ($trow == 3)
                                        <?php $html .= '</tr>'; ?>
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            <tr class="table-active">
                                <td colspan="2" rowspan="2" style="background: white;border: none; text-align: center; width: 310px;">
                                ...
                                @foreach ($months as $value)
                                    ...
                                    <td onclick="viewTablePrice(this)" data-json="<?= htmlspecialchars($table_json, ENT_QUOTES, 'UTF-8') ?>" data-date="{{date('Y-m', strtotime($value['date']))}}" style="text-align: center; cursor: pointer;background-color: #c2fdfdb0; width: 320px;" colspan="3">{{$date->locale('pt_BR')->isoFormat('MMMM')}} {{$date->locale('pt_BR')->isoFormat('YYYY')}} <span class="label label-info float-right">{{$table['code']}}</span></td>
                                    @endif
                                @endforeach
                            </tr>
                            ...
                            @foreach ($category->where('is_conf_cap', 0) as $value)
                                <tr class="table-primary" style="background: black; color: #ffffff;">
                                    <td style="text-align: center" colspan="2">{{$value['name']}}</td>
                                    @foreach ($months as $d)
                                        @if (isset($version_arr[date('Y-m', strtotime($d['date']))]))
                                        <td style="text-align: center" id="category-qtd-total-{{$value['id']}}-{{date('Y-m', strtotime($d['date']))}}">0</td>
                            ...
                            <tr>
                                <td style="text-align: center; background-color: #d9dada" colspan="2">PRAZO PARA PAGAMENTO</td>
                                @foreach ($months as $d)
                                    @if (isset($version_arr[date('Y-m', strtotime($d['date']))]))
