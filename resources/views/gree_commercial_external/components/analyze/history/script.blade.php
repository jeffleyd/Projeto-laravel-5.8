...
function rtd_analyzes(id, namespace) {
    ...
    ajaxSend(
        ...
        $(".rpanel-title").html('Histórico ' + rtd_json.code +' <span onclick="closePanel()"><i class="ti-close right-side-toggle"></i></span>');
        if (rtd_json.status['code']) {
            ...
            if (rtd_json.status['situation'] == 'A solitação foi cancelada!') {
                ...
                list += '<span class="label label-danger" style="font-size: 10px !important;padding: 2px 10px !important;position: relative;bottom: 2px;left: 5px;">Cancelado <span style="font-size: 9px;color: #ffeb5b;">'+rtd_getDateFormat(user.updated_at, true)+'</span> </span></div>';
                list += '<div style="font-size:10px">'+user.office+' </div>';
                list += '<div class="desc" style="background: #fff1e9;padding: 6px;border: solid 2px #d4c3b9;font-size: 12px;">'+user.description+'</div>';
                list += '</div>';
                list += '</div>';
                $(".steamline").html(list);
            ...
            if (rtd_findAnalyzeGroup($val).status) {
                if (rtd_findAnalyzeGroup($val).r_code == $user.r_code) {
                    ...
                    if ($user.users.picture)
                        list += '<div class="sl-left"><span class="badge rounded-pill bg-cyan" style="position: absolute;height: 14px;width: 14px;font-size: 9px;line-height: 1;">'+position+'</span> <img class="img-circle" alt="user" src="'+$user.users.picture+'"> </div>';
                    else
                        list += '<div class="sl-left bg-secondary"><span class="badge rounded-pill bg-cyan" style="position: absolute;height: 14px;width: 14px;font-size: 9px;line-height: 1;">'+position+'</span> <i class="ti-user"></i> </div>';
                    ...
                    list += '<div class="font-medium" style="font-size: 12px;">'+$user.users.short_name;
                    if ($user.is_reprov)
                        list += '<span class="label label-danger" style="font-size: 10px !important;padding: 2px 10px !important;position: relative;bottom: 2px;left: 5px;">Reprovado <span style="font-size: 9px;color: #ffeb5b;">'+rtd_getDateFormat($user.updated_at, true)+'</span> </span></div>';
                    else if ($user.is_approv)
                        list += '<span class="label label-success" style="font-size: 10px !important;padding: 2px 10px !important;position: relative;bottom: 2px;left: 5px;">Aprovado <span style="font-size: 9px;color: #ffeb5b;">'+rtd_getDateFormat($user.updated_at, true)+'</span> </span></div>';
                    else
                    ...
                    var description = $user.description != null ? '<div class="desc" style="background: #fff1e9;padding: 6px;border: solid 2px #d4c3b9;font-size: 12px;">'+$user.description+'</div>' : '<div class="desc"></div>';
