        ...
        <div class="col-md-7 align-self-center text-right">
            <div class="d-flex justify-content-end align-items-center">
                <a class="btn btn-warning d-none d-lg-block m-l-15" onclick="selectProgramation()" href="#">
                    <i class="fa fa-pencil"></i> Alterar programação
                </a>
...
@section('content')
<form action="/comercial/operacao/order/save" id="sendOrder" method="post">
    <input type="hidden" name="programation_id" id="programation_id" @if (count($arr_month)>0) value="{{$month->programation_id}}" @else value="0" @endif>
    <input type="hidden" name="programation_month_id" id="programation_month_id" @if (count($arr_month)>0) value="{{$month->id}}" @else value="0" @endif>
...
<div class="row">
	...
	<div class="col-12">
        <div class="alert alert-danger">
            <b>ATENÇÃO:</b> O mês que escolheu na programação, está com o reajuste 'zerado', fale com seu superior antes de dar andamento na sua solicitação.
			<br><i>Caso esteja errado, por favor, edite sua programação e escolha novamente a condição comercial no mês que deseja atualizar o reajuste.</i>
        </div>
    </div>
	@endif
    <div class="col-12 col-sm-3" style="line-height: 2.2;">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">Informações do pedido programado</h5>
                <div class="row">
                    <div class="col-12">
                        <b>Código:</b> {{$month->programation->code}}<br>
                        <b>Versão:</b> 00{{$month->programation->programationVersionLast->version}}<br>
                        @php $dcarbon = new \Carbon\Carbon($month->yearmonth); @endphp
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-3 form-group ">
                                        ...
                                        <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                                            <input type="text" class="form-control" name="commission" id="commission" placeholder="1.00" value="{{number_format($client->commission, 2, '.', '')}}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <tbody><tr class="table-active">
                                    <td colspan="2" rowspan="2" style="background: #fff3cd;border: none;">
										<div style="border: solid #26d026 2px;background: #98e898;height: 20px;width: 20px;float: left;margin-right: 10px;"></div> 
									<span style="font-size: 12px;">Produtos que fazem parte da programação</span>
                                    </td>
                                </tr>
                                ...
                                @foreach($all_cat_prod as $key)
                                    @foreach($arr_month as $am)
                                        @if ($am['id'] == $key->id)
                                            ...
                                            @foreach($key->set_product_on_group as $prod)
                                                @foreach($am['products'] as $prd)
                                                    @if ($prd['id'] == $prod->id)
														...
														@if ($is_visible)
															@php
																$total_general += $prd['price'] * $prd['max'];
															@endphp
																<tr @if($prd['qtd'] > 0) style="border: solid #26d026 2px; background: #98e898;" @endif>
																	...
																	<td style="text-align: center"> @if ($prod->product_air_evap) @if (substr($prod->product_air_evap->model, -2) == '/I' or substr($prod->product_air_evap->model, -2) == '/O') {{substr($prod->product_air_evap->model, 0, -2)}} @else {{$prod->product_air_evap->model}} @endif @endif</td>
																	<td style="text-align: center" id="cat-{{$key->id}}-product-{{$prod->id}}-qtd"><input data-cat="{{$key->id}}" data-product="{{$prod->id}}" data-max="{{$prd['max']}}" onkeyup="insertValue(this, 1)" class="qtd-td" type="text" value="{{$prd['max']}}" maxlength="4"></td>
																	...
																	<td style="text-align: center" id="cat-{{$key->id}}-product-{{$prod->id}}-total">R$ {{number_format($prd['price'] * $prd['max'], 2, ',', '.')}}</td>
																</tr>
														@endif
                                                    @endif
                                                @endforeach
                                ...
                                @endforeach
                                <tr>
                                    <td colspan="6" style="text-align: right"><b>TOTAL DO PEDIDO PROGRAMADO:</b> <span id="total">R$ {{number_format($total_general, 2, ',', '.')}}</span></td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body wizard-content">
                                <h4 class="card-title">Configurações do pedido programado</h4>
                                ...
                                <form action="#" class="tab-wizard wizard-circle">
                                    ...
                                    <section class="mt-5">
                                        <div class="row text-center">
                                            <div class="col-md-6">
                                                <button type="button" @if (count($arr_month)>0) class="btn btn-success" @else class="btn btn-secondary" @endif style="height: 190px;width: 190px;" id="SelProgramation">
                                                    <i class="ti-layers" style="font-size: 45px;"></i>
                                                    <div style="position: relative;top: 50px;">PROGRAMAÇÃO</div>
                                                </button>
                                                <i class="ti-arrow-right icon_choose" style="position: relative;font-size: 20px;left: 110px;font-weight: bold;"></i>
                                            </div>
                                            <div class="col-md-6">
                                                <button type="button" @if (count($arr_month)>0) class="btn btn-success" @else class="btn btn-secondary isblock" @endif style="height: 190px;width: 190px;" id="SelMonth">
                                                    <i class="ti-calendar" style="font-size: 45px;"></i>
                                                    <div style="position: relative;top: 50px;">MÊS</div>
                                                </button>
                                            </div>
                                        </div>
                                    </section>
...
<div class="modal chooseProgramation" tabindex="2" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" style="display: none; background-color: rgba(0, 0, 0, 0.5);">
    <div class="modal-dialog modal-sm modal-dialog-centered" style="max-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="mySmallModalLabel">Escolha a programação</h4>
            </div>
            <div class="modal-body">
                <div class="col-sm-12 form-group">
                    <label for="client">Programação</label>
                    <select id="modal_sel_programation" class="form-control" style="width:100%" multiple>
                    </select>
                </div>
            </div>
            <div class="modal-footer">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="col-sm-12 form-group">
                    ...
                    <select id="modal_sel_month" class="form-control" style="width:100%" multiple>
                    </select>
                </div>
            </div>
            <div class="modal-footer">
...
@section('page-scripts')
    ...
    <script type="text/javascript">
        ...
        var globalthis;
        var prog_id = 0;
        var step = @if (count($arr_month)>0) 2; @else 1; @endif
		var areas_discount = {
        ...
        $('#SelProgramation').click(function () {
            if (step >= 1)
                $('.chooseProgramation').modal();
        });
        ...
        function choiceOption(op) {
            if (op == 1) {
                if ($('#modal_sel_programation').val() == "") {
                    return $error('Você precisa selecionar a programação.');
                }
                step = 2;
                $('#programation_id').val($('#modal_sel_programation').val());
                $('#SelProgramation').removeClass('btn-secondary').addClass('btn-success');
                $('#SelMonth').removeClass('isblock');
                $('.chooseProgramation').modal('toggle');

                // Reset
                $('#programation_month_id').val('');
                $('#modal_sel_month').val('');
                $('#SelMonth').removeClass('btn-success').addClass('btn-secondary');

            } else if (op == 2) {
                if ($('#modal_sel_month').val() == "") {
                    return $error('Você precisa selecionar o mês da programação.');
                }
                $('#programation_month_id').val($('#modal_sel_month').val());
        ...
        function recuseOption(op) {
            if (op == 1) {
                step = 1;
                $('#programation_id').val('');
                $('#modal_sel_programation').val(null).trigger("change");
                $('#SelProgramation').removeClass('btn-success').addClass('btn-secondary');
                $('#SelMonth').removeClass('btn-success').addClass('btn-secondary');
                ...
                step == 2;
                $('#programation_month_id').val('');
                $('#modal_sel_month').val(null).trigger("change");
                $('#SelProgramation').removeClass('btn-secondary').addClass('btn-success');
                $('#SelMonth').removeClass('btn-success').addClass('btn-secondary');
                $('.chooseMonth').modal('toggle');
            }
        ...
        function insertValue($this, type) {
            /*if (type == 1) {
                if (parseInt($($this).val()) > ($($this).attr('data-max'))) {
                    ...
                    $error('Não é possivel por maior que a quantidade máxima.');
                }
            }*/
        ...
        function choiceProgramation() {
            if ($('#programation_id').val() == 0)
                return $error('Você precisa escolher a programação!');
            else if ($('#programation_month_id').val() == 0)
                return $error('Você precisa escolher o mês da programação');
            else
                window.location.href = '/comercial/operacao/order/new?programation_id='+$('#programation_month_id').val();
        }
        ...
        $(document).ready(function () {
            ...
            $("#modal_sel_programation").select2({
                maximumSelectionLength: 1,
                language: {
                    noResults: function () {
                        return "Sem resultados...";
            ...
            $('#modal_sel_programation').on('select2:select', function (e) {
                var data = e.params.data;
                prog_id = data.id;
                $('#programation_id').val(data.id);
                $('#modal_sel_month').select2('destroy');
                selectMonthBuilder();
                $('.chs_month').show();

            });

            $('#modal_sel_programation').on('select2:unselect', function (e) {
                $('.chs_month').hide();
                $('#modal_sel_month').val(null).trigger("change");
                $('#programation_id').val(0);
            });

            $('#modal_sel_month').on('select2:select', function (e) {
                var data = e.params.data;
                $('#programation_month_id').val(data.id);
            });

            $('#modal_sel_month').on('select2:unselect', function (e) {
            ...
            @if (count($arr_month) > 0)
                $(".js-select23").select2({
                    ...
                    ajax: {
                        url: '/comercial/operacao/select/group/client?id={{$month->programation->client_id}}',
                        data: function (params) {
                            var query = {
                                search: params.term,
                                page: params.page || 1
			...
			bootbox.dialog({
				className: 'my-dialog',
                message: "O mês que escolheu na programação, está com o reajuste 'zerado', fale com seu superior antes de dar andamento na sua solicitação. <br><br>Caso esteja errado, por favor, edite sua programação e escolha novamente a condição comercial no mês que deseja atualizar o reajuste",
                title: "Aviso importante",
                buttons: {
                    main: {
                        label: "Compreendo",
